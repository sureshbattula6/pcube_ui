...
export class OrderProductsComponent implements OnInit {
  ...
  saveProductCartQty(cartItem : any) {
    // console.log(cartItem);

    this.cartProducts = this.cartProducts.filter((item: any) => item.product != cartItem.product);
    this.cartProducts.push(cartItem);
    this.cartProducts = this.cartProducts.filter((item: any) => item.qty != 0);
  ...
  cartProductsList() {
    ...
    this.commonService.openAlert("Items added");
